/*
    Arquivo relacionado às implementações das funções para manipulação de lista ordenada dinâmica.
    data: 16-08-2025.
...
// Implementa inserir().
void inserir(No **lp, int n){
    // Caso 1: lista vazia.
    if(*lp == NULL){
        // chama inserir início.
        inserir_inicio(lp, n);
        return;
    }
    // Caso 2: lista não vazia..
    No* auxiliar = *lp; // cria variável auxiliar para percorrer a lista.
    // Verifica se o elemento é o primeiro da lista.
    if(auxiliar->info > n){
        inserir_inicio(lp, n);
        return;
    }
    for(;auxiliar->info < n && auxiliar->proximo != NULL; auxiliar = auxiliar->proximo); // percorre lista até o fim ou até achar posição correta no meio
    // Verifica se a inserção é no final da lista.
    if(auxiliar->proximo == NULL && auxiliar->info < n){
        inserir_fim(lp, n);
        return;
    }
    // Caso a inserção seja no meio.
    inserir_meio(lp, n, auxiliar->anterior->info);
    return;
