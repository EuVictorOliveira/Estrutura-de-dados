/*
    ...
    de listas ordenadas com com vetor.
...
// Implementa função de inserção ordenada. Faz chamadas das demais funções de inserção a depender da situação
void inserir(Lista *l, int n){
    ...
    // Verifica se a lista está vazia. Nesse caso, a inserção é sempre no início.
    if(lista_vazia(l)){
        insere_inicio(l, n);
        return;
    }
    // Caso para lista não vazia.
    int posicao; // cria variável posição para percorrer a lista.
    for(posicao = 0; l->vetor[posicao] < n && posicao <= l->ctr; posicao++); // percorre a lista até achar ou chegar no fim
    // Caso o elemento seja menor que o primeiro elemento da lista:
    if(posicao == 0){
        insere_inicio(l, n);
        return;
    }
    // caso o elemento seja maior que último elemento da lista:
    if(posicao == l->ctr && n > l->vetor[posicao]){
        insere_final(l, n);
        return;
    }   
    // Caso a inserção seja no meio da lista.
    insere_meio(l, posicao, n);
    return;
}
