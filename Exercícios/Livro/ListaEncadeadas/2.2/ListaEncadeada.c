...
int maiores(No *l, int n){
    // Verifica se a lista está vazia
    if(l == NULL)
        return 0; // retorna -1 caso a lista esteja vazia;

    No *auxiliar; // variável para percorrer a lista
    int maiores = 0; // variável que auxiliar que será retornada.

    for(auxiliar = l; auxiliar != NULL; auxiliar = auxiliar->proximo){ // percorre a lista até o último elemento.
        if(auxiliar->info > n)
            maiores++; // incrementa variável
    }   

    return maiores; // retorna a quantidade de elementos maiores que n na lista.
}
