...
// Implementa concatena()
No* concatena(No* l1, No* l2){
    // Verifica se ambas as listas está vazias
    if(l1 == NULL && l2 == NULL)
        return NULL;

    // Verifica se l1 é vazia
    if(l1 == NULL)
        return l2;

    // Verifica se l2 é vazia
    if(l2 == NULL)
        return l1;
    

    // Caso ambas sejam não vazias
    No* novaLista; // variável que representa
    No* auxiliar; // auxiliar para percorrer l1 copiando a lista

    cria_lista(&novaLista); // cria nova lista
    for(auxiliar = l1; auxiliar != NULL; auxiliar = auxiliar->proximo) // percorre lista 1 até o final inserido elementos na nova lista.
        inserir_fim(&novaLista, auxiliar->info);
    // for para percorrer nova lista até fim:
    for(auxiliar = novaLista; auxiliar->proximo != NULL; auxiliar = auxiliar->proximo);
    auxiliar->proximo = l2; // ultimo elemento da nova lista agora aponta pra l2.
