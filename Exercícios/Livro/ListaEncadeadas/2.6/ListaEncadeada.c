...
No* separa(No* l, int n){
    ...
    No* auxiliar; // cria variável auxiliar para percorrer a lista

    for(auxiliar = l; auxiliar->info != n && auxiliar->proximo != NULL; auxiliar = auxiliar->proximo); // percorre lista até encontrar valor ou chegar ao fim
    // Verifica se chegou ao ultimo elemento, independente de o valor estar presente nele ou não, retorna null
    if(auxiliar->proximo == NULL)
        return NULL;
    // Realiza seperação caso ao contrário
    No* auxiliar2 = auxiliar->proximo; // cria um novo nó auxiliar que aponta para o nó posterior ao auxiliar
    auxiliar->proximo = NULL; // faz auxiliar->próximo apontar para NULL, sendo o último elemento da primeira sublista

    return auxiliar2; // retorna auxiliar2, sendo o primeiro elemento da segunda sublista.
}
